...
### Sub-Workflow: 方剂拆解分析(具体方子 → 拆方) ★新增 2026-07-10
...
- 用户明确说「我看了某医案,想知道这方怎么用」 → 直接调对应医案,跳过 Step 3
- 用户主诉明显(如带状疱疹) → 跳过 Step 5 的反推,直接进入「辨证速查」
- 是熟悉方(小柴胡、五苓散等) → 跳过 Step 3 的 grep,直接走 Step 4-6
...
```
示例(用户处方):
  柴胡10  黄芩10  法半夏10  党参10  甘草6
  苍术15  桂枝10  泽泻30   猪苓10  茯苓30
  麻黄6   薏苡仁60  苦杏仁10

初步分组:
  柴胡、黄芩、法半夏、党参、甘草  →  疑似小柴胡汤骨架
  茯苓、桂枝、泽泻、猪苓          →  疑似五苓散骨架
  麻黄、苦杏仁、薏苡仁、甘草      →  疑似麻杏苡甘汤骨架
  苍术                            →  疑代白术(?)
```

**Pitfall**:不要一次猜方,要先看**结构**(哪几味药反复出现)和**剂量**(重用意味着核心)。

---

#### Step 1 — 拆方拆方(decompose the formula)
逐味药物扫一遍,识别里面**至少 2-3 个经典方骨架**。常见骨架速查表:

**经方底方**:
| 关键药物 | 提示方剂 |
|---|---|
| 柴胡+黄芩+半夏+人参/党参+甘草+生姜+大枣 | 小柴胡汤 |
| 柴胡+黄芩+半夏+党参+甘草+茯苓+桂枝+白术/苍术+泽泻+猪苓 | **柴苓汤**(小柴胡+五苓) |
| 麻黄+桂枝+杏仁+甘草 | 麻黄汤 |
| 桂枝+白芍+生姜+大枣+甘草 | 桂枝汤 |
| 茯苓+桂枝+白术/苍术+甘草 | 苓桂术甘汤 |
| 茯苓+桂枝+白术/苍术+泽泻+猪苓 | 五苓散 |
| 麻黄+杏仁+薏苡仁+甘草 | 麻杏苡甘汤(湿家身痛) |
| 麻黄+杏仁+石膏+甘草 | 麻杏石甘汤(寒包火) |
| 麻黄+附子+细辛 | 麻黄附子细辛汤(少阴表证) |
| 附子+茯苓+苍术/白术+生姜+赤芍 | 真武汤 |
| 桂枝+白芍+生姜+大枣+甘草+附子 | 桂枝加附子汤 |
| 桂枝+茯苓+苍术+炙甘草+黄连+细辛+车前子 | **明朗饮**(苓桂术甘+加味) |
| 升麻+醋鳖甲+当归+甘草+雄黄/蜀椒 | 升麻鳖甲汤(阴阳毒/带疱) |
| 全瓜蒌+红花+生甘草 | 瓜蒌红花汤(带疱专病专方) |

**时方/经验方**:
| 关键药物 | 提示方剂 |
|---|---|
| 蝉蜕+僵蚕+姜黄+大黄 | 升降散(杨栗山) |
| 杏仁+白蔻仁+薏苡仁+厚朴+半夏+通草+滑石+竹叶 | 三仁汤(吴鞠通) |
| 槟榔+厚朴+草果+黄芩+知母+白芍+甘草 | 达原饮(吴又可) |
| 玄参+牡蛎+浙贝母 | 消瘰丸(程钟龄) |
| 桂枝+白术+茯苓+甘草+干姜+厚朴+陈皮+白芍 | **理饮汤**(张锡纯·寒饮停肺) |
| 桂枝+茯苓+桃仁+丹皮+赤芍 | 桂枝茯苓丸 |

**医林独啸斋高频经验药对/角药**(若成对出现 = 该病机):
| 药对/角药 | 病机信号 |
|---|---|
| 款冬花+紫菀+苦杏仁 | 止咳化痰对症群 |
| 石菖蒲+鸡内金+山楂 | 化湿浊+开胃(医林标配) |
| 石菖蒲+佩兰 | 化湿浊、新冠系列标配 |
| 丹参+丹皮 | 凉血祛瘀(化学品灼伤案标配) |
| 全蝎+蜈蚣 | 搜剔瘀血+以毒攻毒保护神经(带疱神经痛) |
| 板蓝根 | 邹孟城带疱加味 |
| 升麻+鳖甲+当归 | 托疹透毒(带疱、阴阳毒) |

**关键技巧**:
- **剂量重用者往往是核心药**(比如茯苓 30g、薏苡仁 60g → 必为核心方之一)
- **同名药对/角药 = 经验组合信号**
- **缺原方某药 = 必有替代或简化**(常见:去大黄用枳实+厚朴、去白术用苍术、去姜枣)
- **不熟悉的小众方不要硬猜** → 进入 Step 3 用 grep 反查

**Pitfall**:注意 ripgrep 的 `-e` 与 OR 语法:`rg -l "升降散"`(直接) vs `rg -l "三仁汤|消瘰丸"`(错)→ 应用两次 `-l`。

---

#### Step 2 — 锁定核心方
识别出**最重要的 1-2 个经典方**(占药物最多、或最体现整体病机的那张)。后续围绕它展开。

---

#### Step 3 — Grep 反查(可选,但对不熟悉方强烈推荐)

对每一个识别出的核心方,用 `ripgrep` 在医林独啸斋语料库中反查:

```bash
rg -l "核心方名" /Users/applemima1111/AiCoding/医林独啸斋/markdown/
```

**优先级**:
1. 直接命名该方的医案(如「麻杏柴苓汤一剂热退人安」)→ 最权威
2. 涉及该方的医案(合方/加减方)→ 次权威
3. references/ 中已有的专题文件(如 `damp_heat_three_formulas.md`、`sanren_tang_formula.md`)→ 已固化的解读

**已知反例与边界**:
- **自拟方无经典骨架**:遇到完全找不到骨架的方,不要硬凑——承认「这是医家自拟方」,转而分析其药物配伍思路
- **同名异方**:如「理饮汤」(张锡纯)与一般「理饮」思维不同,要明确出处
- **跨方合用**:多个方同时出现,合理猜测,不要全锁死一个
- **剂量逆推病机**:重用某药意味着该病机重(茯苓30g + 泽泻30g + 薏苡仁60g = 湿浊重)

---

#### Step 4 — 三大方逐个对位表(必须)
每个识别出的方剂都要做**原方 vs 你的方**对照表:
```
你方              方原方          备注
──────────      ──────────       ────────
药A ✓           药A              同(重用)
...
#### Step 6 — 推论主治病机 + 回归到原方(必须收口)
**反向推论**(从拆出的方剂反推这个方最可能治什么病机):
```
┌─────────────────────────┐
│ 病机分条                 │
│ ① 气机不畅(主方A治)     │
...
**结尾必须有「一句话核心心法」**——这是 Erik 的硬性偏好,作为收束。

**最后永远问用户**:「有具体主诉吗?可以精修推论,也能帮你反查用药是否需要加减」

---

#### Step 7 — 关键加减的解读(为什么这样减/换)
对**每一个偏离原方的药**给出推断:
- 「去大黄」 → 可能患者脾胃虚弱
- 「苍术代白术」 → 湿浊较重、脾虚不显
- 「重用某药 X 倍」 → 该药证更突出
...
#### Step 8 — 医林独啸斋中的对应锚点(强差异化)
不要只列「本方含 X、Y、Z」。必须**给出对应的 references 文件名或文章标题**,让 Erik 一眼能反查:
```
核心对应文章:
  → 「具体文章标题」 (line X-Y)
  → references/<topic>.md
...
#### Step 9 — 是否要蒸馏成新专题(decision point)
**判断标准**:
- ✅ 如果这个方剂组合在医林独啸斋有 ≥3 篇独立医案 → 考虑蒸馏成 references/<topic>.md
- ❌ 如果只是临时拆方,且没有独立专题支撑 → 不必蒸馏,直接输出即可

**如果决定蒸馏**:
- 文件命名:`references/<topic>_formula.md` 或 `<topic>_decomposition.md`
...
Erik 偏好:**纯文本 + ASCII 结构 + 引证带 line number**。
